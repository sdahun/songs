...
use \sdahun\songs\converter\Xml2Pptx;
use \sdahun\songs\converter\XmlConfigurator;

class PowerPointWriter extends AbstractWriter {
    ...
    private function startNewOutput() {
        ...
        echo("  A PowerPoint fájlok a következő fájlba kerülnek:\n    /" . basename($path) . '/' . $fname . "\n");
    }
    ...
    public function addSongFile($xml_path) {
        if (file_exists($xml_path)) {
            ...
            $this->zip->addFromString (
                basename (dirname ($xml_path)) . '/' . basename ($xml_path, '.xml') . '.pptx',
                Xml2Pptx::convert (XmlConfigurator::configure (file_get_contents ($xml_path), $this->prefs))
            );
        }
    }
}
